...
export function TemplateEditor2({
	...
	const handleLoaded = () => {
		...
		const body =
			docs.querySelector("body") || docs.querySelector("[role=article]");
		if (!body) {
			return;
		}
		docs.addEventListener("mousemove", (e) => {
			...
			if (!e.target.hasAttribute("data-element-id")) {
				...
				e.target.setAttribute(
					"data-element-id",
					`${e.target.tagName}-${targetId}`,
				);
			}

			if (
				docs.querySelector(
					`div[data-target-id=${e.target.getAttribute("data-element-id")}`,
				)
			) {
				return;
			}
		...
		docs.addEventListener("click", (e) => {
			if (e.target.className.indexOf("overlay-current-inspect-element") >= 0) {
				const targetBlock = docs.querySelector(
					`[data-element-id=${e.target.getAttribute("data-target-id")}]`,
				);
				...
				openVariableAssignmentPopover({
					...
					originalTextContent: targetBlock.getAttribute(
...
export function VariableAssignPopover({
	form,
	targetRect,
	offsetElementRef,
	onSet,
	...
	useEffect(() => {
		// position below the static variables input, then prefill + focus
		if (!targetRect || !popoverModal.current) {
			return;
		}
			...
			<div class="content p-3">
				...
				<div class="assign-block mt-2">
					<form
						onSubmit={(e) => {
							e.preventDefault();
							onSet(assignInput.current.value);
						}}
					>
						<input
							ref={assignInput}
