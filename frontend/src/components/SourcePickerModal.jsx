...
export function SourcePickerModal({
	...
	const getUsedByField = (sourceValue) => {
		const mapping = existingMappings.find(
			(m) => m.source === sourceValue && m.target !== currentTarget,
		);
			...
			<div className="flex-1 min-w-0">
				...
				{isUsed && (
					<span className="block text-xs text-gray-500 mt-0.5">
						already used by{" "}
						<span className="font-medium text-gray-700">{field.usedBy}</span>
					</span>
				)}
			</div>
