...
export function TemplateEditor() {
	...
	const cleanupOverlays = () => {
		...
		container
			.querySelectorAll(
				".block-tag-overlay, .block-tag-border, .block-tag-badge",
			)
	...
	const extractFieldsFromTemplate = useCallback(async (templateText) => {
		...
		try {
			const response = await api.extractTemplateFields({
	...
	useEffect(() => {
		setTemplate((prev) =>
			prev.template === fullTemplateText
				? prev
				: { ...prev, template: fullTemplateText },
		);
	}, [fullTemplateText]);
		...
		try {
			...
			if (id) {
				...
				if (t.fields && Array.isArray(t.fields)) {
					...
					const assignments = t.fields.filter(
						(f) => f.kind === "static_assign",
					);
				...
				if (selectedConnectionId) {
					try {
						await api.createTrigger({
							...
							is_active: true,
							field_mappings: selectedConnectionFields
								.filter((f) => f.isMapped)
								.map((f) => ({
									source: f.source,
									target: f.name,
	...
	const removeField = (fieldName) => {
		...
		if (referenceEmail) {
			setReferenceEmail((prev) =>
				prev ? { ...prev, body: prev.body.replace(regex, "") } : null,
			);
		}
	};
	...
	const addVariableAssignment = () => {
		...
		if (varMapWithConnection) {
			setSelectedConnectionFields((prev) =>
				prev.map((f) =>
					f.name == name
						? { ...f, isMapped: true, source: `\$extracted_data.${name}` }
						: f,
				),
			);
		}
	...
	const removeVariableAssignment = (index) => {
		...
		const nextLines = lines.filter(
			(line) => !line.match(new RegExp(`^\\{%\\s*${assignment.name}\\s*=`)),
		);
	...
	const handleTextSelection = () => {
		...
		if (selectionMode === "block" && template.content_type === "text/html")
			return;
	...
	const handleAddVariableFromSelection = () => {
		...
		if (varMapWithConnection) {
			setSelectedConnectionFields((prev) =>
				prev.map((f) =>
					f.name === fieldName
						? { ...f, isMapped: true, source: `\$extracted_data.${fieldName}` }
						: f,
				),
			);
		}
		...
		const marker =
			dataType === "str" ? `{{${fieldName}}}` : `{{${fieldName}:${dataType}}}`;
		...
		if (referenceEmail) {
			setReferenceEmail((prev) =>
				prev
					? { ...prev, body: prev.body.replace(selectedText, marker) }
					: null,
			);
		}
	...
	const showHoverOverlay = (targetEl) => {
		...
		if (!overlay) {
			...
			overlay.style.cssText =
				"position:absolute;pointer-events:none;z-index:10;border:1px dashed rgba(37,99,235,0.35);background:rgba(37,99,235,0.06);border-radius:2px;transition:opacity 80ms ease;opacity:0;";
	...
	const hideHoverOverlay = () => {
		const overlay = emailViewerRef.current?.querySelector(
			".block-tag-hover-overlay",
		);
	...
	const showSelectionOverlay = (targetEl) => {
		...
		if (!overlay) {
			...
			overlay.style.cssText =
				"position:absolute;pointer-events:none;z-index:10;border:2px solid #2563EB;background:rgba(239,246,255,0.4);border-radius:2px;";
			container.appendChild(overlay);
		}
		positionOverlay(overlay, targetEl);
	};
	...
	const cleanupSelectionOverlay = () => {
		emailViewerRef.current
			?.querySelector(".block-tag-selection-overlay")
			?.remove();
	};
	...
	const buildTemplateFromDOM = (targetElementId, newContent) => {
		...
		const cloneEl = clone.querySelector(
		...
		clone
			.querySelectorAll(
				".block-tag-hover-overlay, .block-tag-selection-overlay, .block-tag-border, .block-tag-badge",
			)
			.forEach((el) => el.remove());
	...
	const confirmBlockTag = () => {
		...
		if (
			extractedFields.some((f) => f.name === name) &&
			!blockTaggedElements[selectedElementId]
		) {
			// Field exists but we're not editing an existing tag on this element
			const existingEl = Object.entries(blockTaggedElements).find(
				([, v]) => v.fieldName === name,
			);
		...
		// Get element content
		const el = emailViewerRef.current?.querySelector(
		...
		if (referenceEmail) {
			setReferenceEmail((prev) =>
				prev ? { ...prev, body: newTemplateText } : null,
			);
		}
	...
	const removeBlockTagByElementId = (elementId) => {
		...
		// replacement bugs.
		const newTemplateText = buildTemplateFromDOM(
			elementId,
			tagInfo.originalContent,
		);
	...
	const removeBlockTagByFieldName = (fieldName) => {
		...
		const entries = Object.entries(blockTaggedElements).filter(
		...
		entries.forEach(([elementId, tagInfo]) => {
			const tempResult = buildTemplateFromDOM(
				elementId,
				tagInfo.originalContent,
			);
			if (tempResult !== null) {
	...
	const renderBlockOverlays = () => {
		...
		// Clean old
		container
			.querySelectorAll(".block-tag-border, .block-tag-badge")
			.forEach((el) => el.remove());
	...
	const highlightVariables = (text) => {
		...
		const pattern = extractFieldNames
			.map((f) => `\\{\\{${f}(?::\\w+)?\\}\\}`)
			.join("|");
		...
		return text.split(regex).map((part, index) => {
			...
			const isVariable = extractFieldNames.some(
				(f) => part === `{{${f}}}` || part.startsWith(`{{${f}:`),
			);
			if (isVariable) {
				return (
					<span
						key={index}
						className="bg-yellow-200 px-1 rounded font-medium text-blue-700"
					>
						{part}
					</span>
				);
	...
	const renderReference = () => {
		...
		if (template.content_type === "text/html") {
			...
			extractedFields.forEach((field) => {
				...
				const escapedMarkerWithType = markerWithType.replace(
					/[.*+?^${}()|[\]\\]/g,
					"\\$&",
				);
						...
						<div className="flex items-center gap-2 mb-2 px-1">
							...
							{selectionMode === "text" && (
								<span className="text-xs text-gray-500">
									Select text to create variables
								</span>
							)}
						</div>
					)}
						...
						<div className="p-4 h-[calc(100%-49px)]">
							{activeTab === "reference" && (
								<div className="h-full flex flex-col">
									<div className="flex-1 relative">{renderReference()}</div>
								</div>
							)}
							{activeTab === "template" && (
								<div className="h-full flex flex-col">
					...
					<div className="col-span-4 space-y-4">
									...
									<div>
										...
										{!selectedConnectionId && (
											<p className="text-xs text-gray-500 mt-1">
												Selecting a connection will auto-create a trigger after
												saving.
											</p>
										)}
									</div>
						...
						<div className="bg-white rounded-lg border border-gray-200 p-4">
							...
							<div className="space-y-2">
								{[
									...
									<div
										key={index}
										className={`flex items-center justify-between p-2 rounded ${field.kind === "static_assign" ? "bg-blue-50" : "bg-gray-50"}`}
									>
										<div className="flex items-center gap-2">
											<code className="text-sm">{field.name}</code>
											<span
												className={`text-xs px-2 py-0.5 rounded ${field.kind === "static_assign" ? "bg-blue-200 text-blue-800" : "bg-gray-200 text-gray-700"}`}
											>
												{field.type}
											</span>
											<span
												className={`text-xs px-2 py-0.5 rounded ${field.kind === "static_assign" ? "bg-blue-100 text-blue-700" : "bg-yellow-100 text-yellow-700"}`}
											>
												{field.kind}
											</span>
										</div>
										<button
											onClick={() => {
												if (field.kind === "static_assign") {
													...
													const blockEntry = Object.entries(
														blockTaggedElements,
													).find(([, v]) => v.fieldName === field.name);
													if (blockEntry) {
						...
						{id && (
							<div className="bg-white rounded-lg border border-gray-200 p-4">
								...
								{testResult && (
									<div
										className={`mt-3 p-3 rounded text-sm ${testResult.success ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
									>
										{testResult.success ? (
							...
							{!varMapWithConnection && (
								<div className="mb-4">
									...
									<input
										...
										className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
										onKeyPress={(e) =>
											e.key === "Enter" && addVariableAssignment()
										}
										autoComplete="off"
									/>
								</div>
