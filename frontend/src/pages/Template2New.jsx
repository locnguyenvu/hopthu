...
export function Template2New() {
	...
	const setVariable = async (variableName, popOverForm) => {
		...
		const targetElement = docs.querySelector(
			`[data-element-id=${popOverForm.targetId}]`,
		);

		const replacedHtml = popOverForm.htmlContent.replace(
			popOverForm.textContent.trim(),
			variableName,
		);
	...
	const clearVariable = async (popOverForm) => {
		...
		targetElement.innerText = targetElement.getAttribute(
			"data-original-content",
		);
	...
	const parseTemplate = async (replacements) => {
		...
		for (const [_, replacement] of Object.entries(replacements)) {
			if (templateStr.includes(replacement[0])) {
				...
				const elementSearch = domParser.parseFromString(
					replacement[0],
					"text/html",
				);
				const elementReplace = domParser.parseFromString(
					replacement[1],
					"text/html",
				);
		...
		const result = await api.extractTemplateFields({
	...
	const handleCreate = async () => {
		...
		try {
			const parseResult = await api.extractTemplateFields({
				template: templateOutput,
			});
					...
					<div class="flex gap-3 h-full px-10 min-w-4xl max-w-8xl mx-auto">
						<div class="grow w-2/3">
							...
							<div class="relative h-svh scrollbar-none">
								...
								<textarea
									...
									style={{
										display: previewType === "raw" ? "block" : "none",
										height: "100%",
									}}
						...
						<div class="w-1/3 flex flex-col gap-3">
							<div class="p-2 border-1 border-neutral-100 shadow-sm rounded-sm">
								...
								<form
									ref={templateAttributeForm}
									id="templateAttribute"
									class="flex flex-col gap-3 px-1 mt-2 "
								>
							...
							<div class="flex gap-2">
								<button
									class="p-1 rounded-sm bg-gray-200 hover:bg-grey-100 text-black text-xs w-full"
