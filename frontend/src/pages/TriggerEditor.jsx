...
export function TriggerEditor() {
	...
	const unmappedCount = fieldMappings.filter(
		(m) => !m.source && !m.required,
	).length;
	const requiredMissingCount = fieldMappings.filter(
		(m) => !m.source && m.required,
	).length;
	...
	const generateAutoMappings = () => {
		const mappings = connectionFields.map((connField) => {
			...
			return {
				source: matchedTemplateField
					? `$extracted_data.${matchedTemplateField.name || matchedTemplateField}`
					: "",
		...
		toast.success(
			`Auto-mapped ${autoMappedCount} of ${mappings.length} fields`,
		);
	};
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (isEdit) {
				...
				setLocation(`/triggers/${result.data.id}`);
			}
		} catch (e) {
			...
			return (
				<nav className="text-sm text-gray-500 mb-4">
					...
					<span className="text-gray-700">
						{isEdit ? name || "Edit Trigger" : "New Trigger"}
					</span>
				</nav>
			);
		}
								...
								<label className="flex items-center gap-3 cursor-pointer">
									...
									<div>
										...
										<p className="text-xs text-gray-500">
											Trigger will process incoming emails when active
										</p>
									</div>
								</label>
							</div>
						...
						<div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-200">
							<div>
								...
								<p className="text-xs text-gray-500 mt-1">
									Map extracted data fields to connection payload fields
								</p>
							</div>
							...
							<div className="text-center py-8 bg-gray-50 rounded-lg">
								...
								<p className="text-gray-600 mb-1">
									{connectionId
										...
										: "Select a template and connection to configure mappings"}
								</p>
								{connectionId && connectionFields.length === 0 && (
									<p className="text-sm text-gray-400">
										Define fields in your connection to see them here
									</p>
								)}
							</div>
						)}
