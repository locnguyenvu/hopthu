...
function MethodBadge({ method }) {
	...
	return (
		<span
			className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${colors[method] || "bg-gray-100 text-gray-700"}`}
		>
			{method}
		</span>
	);
	...
	const handleTest = async (id) => {
		try {
			...
			if (result.data.success) {
				toast.success(
					`Connection test passed (${result.data.response_status})`,
				);
			} else {
					...
					<Card>
						<CardContent className="p-12 text-center">
							...
							<p className="text-[var(--color-muted-foreground)] mb-6">
								Create a connection to integrate with external systems
							</p>
							<Link href={"/connections/new"}>
								<Button>
									<Plus className="w-4 h-4" />
