...
export function Template2DetailEditor() {
	...
	const setVariable = (variableName, popOverForm) => {
		...
		setTemplate({
			...template,
			template: template.template.replace(originalText, variableName),
		});
	};
	...
	const clearVariable = (popOverForm) => {
		...
		if (
			!targetElement ||
			!targetElement.hasAttribute("data-original-content")
		) {
			return;
		}
		...
		setTemplate({
			...template,
			template: template.template.replace(assignedText, originalText),
		});
	};
					...
					<div class="flex gap-3 h-full px-10 min-w-4xl max-w-8xl mx-auto">
						<div class="grow w-2/3">
							<div
								...
								class="py-2 flex place-content-between border-b-2"
							>
