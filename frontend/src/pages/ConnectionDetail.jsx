...
function ConnectionEditForm({ id, connection, onSaved }) {
	...
	useEffect(() => {
		if (connection) {
			...
			setHeaders(
				connection.headers || [{ key: "", value: "", encrypted: false }],
			);
			setFields(
				connection.fields || [{ name: "", type: "string", required: false }],
			);
		}
	}, [connection]);
					...
					<div key={index} className="flex gap-2 mb-2">
						...
						<label className="flex items-center gap-1 text-sm text-gray-600">
							<input
								...
								onChange={(e) =>
									updateHeader(index, "encrypted", e.target.checked)
								}
			...
			{testResult && (
				<div className="mb-4 p-4 bg-gray-50 rounded-lg">
					...
					<div className="flex items-center gap-2 mb-2">
						<span
							className={`px-2 py-1 rounded text-sm ${testResult.success ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
						>
	...
	return (
		<div>
			<div className="flex justify-between items-center mb-4">
				...
				<button
					onClick={() =>
						setLocation(`/triggers/new?connection_id=${connectionId}`)
					}
